...
class Graph:
    def __init__(self, base_stations, clients, slice_variation, xlim, map_limits, PLOT):
        ...
        self.gs6 = gridspec.GridSpec(1, 1)
    ...
    def draw_map(self):
        ...
        for c in self.clients:
            ...
            if c.get_slice().name.split('_')[0] not in legend_indexed and c.base_station is not None:
                ...
                legend_indexed.append(label)
            self.ax.scatter(c.x, c.y,
                            color = "b" if 'S1' in c.get_slice().name else "g",
                            label=label, s=20,
                            #marker=markers[c.subscribed_slice_index % len(markers)]
                            marker="o" if 'S1' in c.get_slice().name else "s")
    ...
    def draw_stats(self, **stats):
        ...
        def capacity_usage_graph(bs_num, x_cap, y_cap):
            ...
            if self.PLOT['label']:
                ...
                for i in self.slice_variation:
                    self.ax.plot(stats[f'AL_usage_bs{bs_num}_{i}'])
                    self.ax.plot(stats[f'BL_usage_bs{bs_num}_{i}'])
        ...
        def load_ratio_graph(bs_num, x_cap, y_cap):
            ...
            if self.PLOT['label']:
                ...
                for i in self.slice_variation:
                    self.ax.plot(stats[f'AL_load_ratio_bs{bs_num}_{i}'])
                    self.ax.plot(stats[f'BL_load_ratio_bs{bs_num}_{i}'])
        ...
        def UE_count(bs_num, x_cap, y_cap):
            ...
            if self.PLOT['label']:
                ...
                for i in self.slice_variation:
                    self.ax.plot(stats[f'AL_c_count_bs{bs_num}_{i}'])
                    self.ax.plot(stats[f'BL_c_count_bs{bs_num}_{i}'])
            ...
            self.ax.use_sticky_edges = False
            if bs_num == 0:
                self.ax.set_title('The number of UEs using donor')
            else:
                self.ax.set_title(f'The number of UEs using node{bs_num}')
        ...
        def bandwidth_allocation(bs_num, x_cap, y_cap):
            ...
            if self.PLOT['label']:
                ...
                for i in self.slice_variation:
                    self.ax.plot(stats[f'AL_bandwidth_allocation_bs{bs_num}_{i}'])
                    self.ax.plot(stats[f'BL_bandwidth_allocation_bs{bs_num}_{i}'])
            self.ax.set_xlim(self.xlim)
            locs = self.ax.get_xticks()
            locs[0] = self.xlim[0]
            locs[-1] = self.xlim[1]
        ...
        """
        def route_check():
            self.ax = plt.subplot(self.gs6[0, 0])
            plt.xlabel("time")
            plt.ylabel("count")
            self.ax.plot(stats['one_zero'], label=f'1, 2, donor')
            self.ax.plot(stats['two_zero'], label=f'2, donor')
            self.ax.plot(stats['zero'], label=f'donor')
            self .ax.plot(stats['whole'], label=f'whole')
            self.ax.set_xlim(self.xlim)
            locs = self.ax.get_xticks()
            locs[0] = self.xlim[0]
            locs[-1] = self.xlim[1]
            ...
            self.ax.legend(loc="lower right")
            self.ax.use_sticky_edges = False
            self.ax.set_title("UEs' route to donor")
        ...
        plt.tight_layout()
        plt.savefig("graph/route_check.png", dpi=1000)
        """
